CANYON_AT <- function(date,lat,lon,pres,temp,psal,doxy){
  ...
  data <- cbind(lat/90,sinpi(lon/180),cospi(lon/180),cospi(day/180)*fsigmoid,sinpi(day/180)*fsigmoid,temp,psal,doxy,pres/2e4+1/((1+exp(-pres/300))^3))
  
  temporaire <- read.table(paste(basedir,"Fichier_poids_AT_hidden_ascii19_17.sn",sep=""))
  poids <- temporaire[,3]
  rm(temporaire)
  ...
  Moy <- unlist(read.table(paste(basedir,"moy_AT.dat",sep="")),use.names = "FALSE")
  Ecart <- unlist(read.table(paste(basedir,"std_AT.dat",sep="")),use.names = "FALSE")
