CANYON_PO4 <- function(date,lat,lon,pres,temp,psal,doxy){
  ...
  temporaire <- read.table(paste(basedir,"Fichier_poids_PO4_hidden_ascii17_17.sn",sep=""))
  poids <- temporaire[,3]
  rm(temporaire)
  ...
  Moy <- unlist(read.table(paste(basedir,"moy_PO4.dat",sep="")),use.names = "FALSE")
  Ecart <- unlist(read.table(paste(basedir,"std_PO4.dat",sep="")),use.names = "FALSE")
