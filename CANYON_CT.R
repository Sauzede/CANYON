CANYON_CT <- function(date,lat,lon,pres,temp,psal,doxy){
  ...
  temporaire <- read.table(paste(basedir,"Fichier_poids_CT_hidden_ascii18_15.sn",sep=""))
  poids <- temporaire[,3]
  rm(temporaire)
  ...
  Moy <- unlist(read.table(paste(basedir,"moy_CT.dat",sep="")),use.names = "FALSE")
  Ecart <- unlist(read.table(paste(basedir,"std_CT.dat",sep="")),use.names = "FALSE")
