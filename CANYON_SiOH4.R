CANYON_SiOH4 <- function(date,lat,lon,pres,temp,psal,doxy){
  ...
  temporaire <- read.table(paste(basedir,"Fichier_poids_SI_hidden_ascii20_15.sn",sep=""))
  poids <- temporaire[,3]
  rm(temporaire)
  ...
  Moy <- unlist(read.table(paste(basedir,"moy_SI.dat",sep="")),use.names = "FALSE")
  Ecart <- unlist(read.table(paste(basedir,"std_SI.dat",sep="")),use.names = "FALSE")
