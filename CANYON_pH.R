CANYON_pH <- function(date,lat,lon,pres,temp,psal,doxy){
  ...
  #
  # output:
  # out   - pH (total scale at insitu PTS)
  #
  ...
  temporaire <- read.table(paste(basedir,"Fichier_poids_PH_hidden_ascii19_8.sn",sep=""))
  poids <- temporaire[,3]
  rm(temporaire)
  ...
  Moy <- unlist(read.table(paste(basedir,"moy_PH.dat",sep="")),use.names = "FALSE")
  Ecart <- unlist(read.table(paste(basedir,"std_PH.dat",sep="")),use.names = "FALSE")
